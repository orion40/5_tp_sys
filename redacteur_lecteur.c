...
void initialiser_lecteur_redacteur(lecteur_redacteur_t * lr){
    fprintf(stderr, "Redacteurs en priorité...\n");
...
void debut_redaction(lecteur_redacteur_t * lr){
    ...
    // Pour assurer la priorité des rédacteurs, il faut maintenant
    // savoir s'il y en a en attente.
    lr->nb_redacteur_attente++;
...
void fin_redaction(lecteur_redacteur_t * lr){
    ...
    // s'il y a des rédacteurs en attente, ils auront la priorité.
    if (lr->nb_redacteur_attente > 0){
        pthread_cond_signal(&lr->notif_ecrivain);
    } else {
